...
export class TgbotService {
  ...
  constructor(
    ...
    if (!process.env.IGNORE_BOT || true) {
      this.logger.log(process.env.TG_BOT_CLIENT_TOKEN);
  ...
  private async onMessage(
    ...
    if (chat && chat.userId) {
      bot.sendMessage(
        id,
        'Эта учетная запись уже связана с учетной записью FitnessHub!',
      );
      return;
    }
    // send token to user
    bot.sendMessage(id, 'Ваш токен:');
