...
export class RecipeFormValidator extends FormValidator {
    ...
    return nut.map((x, i) => {
      return {
        ...
        amount: this.checkNumberMinMax(
          // prettier-ignore
          this.checkNumber(x.amount, 'Количество питательной ценности ' + x.title + ' отсутствует'),
          -1 / 100000000000,
          Number.MAX_VALUE,
          // prettier-ignore
  ...
  public static validate(dto: CreateRecipeDTO, categories: string[]): CreateRecipeDTO {
    ...
    this.checkString(title, 'Пожалуйста, введите название');
    this.checkString(thumbnail, 'Пожалуйста, предоставьте изображение');
    ...
    // prettier-ignore
    steps = this.checkStringArray(steps, 'Укажите этапы подготовки');
