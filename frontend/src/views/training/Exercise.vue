  ...
  <div class="view-Exercise">
    ...
    <FHSwipeable @swipeDown="$cFS('training')">
      <tc-hero :dark="$store.getters.darkmode">
        ...
        <tl-flow v-else flow="column">
          ...
          <p>Упражнение не удалось загрузить</p>
        </tl-flow>
      </tc-hero>
    </FHSwipeable>
    <div content v-if="exercise">
      ...
      <div max-width>
        ...
        <FHHeading title="Мышцы" subtitle="затронутая тема" />
        <FHVarList>
          <FHVarListItem
        ...
        <template v-if="exercise.warnings && exercise.warnings.length > 0">
          <br />
          <FHHeading title="Примечания" subtitle="авторы" />
          <tc-quote
            :dark="$store.getters.darkmode"
        ...
        <template v-if="exercise.steps && exercise.steps.length > 0">
          <br />
          <FHHeading title="Версия" subtitle="правильно" />
...
export default class Exercise extends Vue {
  ...
  get youtubeEmbed(): string | null {
    ...
    const { explanatoryVideo } = this.exercise;

    if (!explanatoryVideo) return null;
    if (!explanatoryVideo.startsWith('https://www.youtube.com/watch?v='))
      return null;

    const videoId = explanatoryVideo.split('v=')[1].split('&')[0];
