    ...
    <FHSwipeable @swipeDown="$cFS('nutrition')">
      <tc-hero :dark="$store.getters.darkmode">
        ...
        <tl-flow v-else flow="column">
          ...
          <p>Рецепт не удалось загрузить</p>
        </tl-flow>
      </tc-hero>
    </FHSwipeable>
      ...
      <div max-width>
        <tl-flow horizontal="space-around" class="quick-information">
          <tl-flow flow="column" class="information">
            ...
            <div class="value">{{ recipe.time }} мин</div>
          </tl-flow>
          <tl-flow flow="column" class="information">
        ...
        <FHAppear>
          <div v-if="(recipe.nutrition || []).length > 0">
            ...
            <FHHeading subtitle="подробнее" title="Пищевая ценность" />
            <div
        ...
        <FHAppear>
          <div v-if="(recipe.ingredients || []).length > 0">
            <FHHeading subtitle="требуется" title="Ингредиенты" />
            <tc-table
              :selectable="true"
              :multiSelect="true"
              :dark="$store.getters.darkmode"
        ...
        <FHAppear>
          <div v-if="(recipe.steps || []).length > 0">
            <tl-flow horizontal="space-between">
              <FHHeading subtitle="Шаги" title="Готовка" />
              <div>
                <tc-button
                  icon="chevron-left"
                  tfbackground="success"
      ...
      .then(async res => {
        ...
        if (!this.recipe) {
          NotificationManagement.sendNotification(
            'Рецепт не найден',
            'Указанный рецепт не существует'
          );
