      ...
      <FHAppear>
        <div :key="index" class="sets-reps" v-if="type === 'gym'">
          ...
          <tc-table :dark="$store.getters.darkmode">
            ...
            <tc-tr v-for="(_x, i) in stats[index].reps" :key="i">
              ...
              <tc-td>{{ stats[index].weights[i] }} кг</tc-td>
            </tc-tr>
          </tc-table>
        ...
        <div :key="index" class="time" v-else-if="type === 'time'">
          ...
          <tl-flow horizontal="space-between">
            <h3>Цель по открытому времени</h3>
          ...
          <div class="grid">
            ...
            >
              <tc-select-item title="Секунды" />
              <tc-select-item title="Минуты" />
              <tc-select-item title="Часы" />
            </tc-select>
            <tl-flow vertical="end">
              <tc-button
                icon="plus"
...
export default class RunningWorkout extends Vue {
  ...
  public save(): void {
    const to = undefined;
    const from = this.stats.length - 1;

    this.indexChanged(to, from);
    clearInterval(this.timer);
    console.log('Данные', this.stats);
