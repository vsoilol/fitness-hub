<template>
  <div class="view-friend-details">
    <FHHeader
      backTitle="Друзья"
      rootRoute="friends"
      :title="friend ? friend.username : 'Друг'"
      :trigger="150"
    >
    ...
    <tc-hero :dark="true" :hasFixedHeader="false">
      ...
      <FHAppear>
        <tl-flow v-if="friend" flow="column">
          ...
          <div v-if="memberSince">Участник с: {{ memberSince }}</div>
        </tl-flow>
    ...
    <div content max-width>
      ...
      <FHAppear>
        <div v-if="exercises">
          <br />
          <br />
          <FHHeading title="Упражнения" subtitle="Последние 10" />
          <p v-if="exercises.length === 0">
            На данный момент ни одна тренировка не была введена
          </p>
          <FHCarousel v-else>
            <FHExercisePreview
      ...
      <FHAppear>
        <div v-if="stats">
          <br />
          <FHHeading title="Спорт" subtitle="Статистика" />
