<template>
  <div class="view-recipe-form" content>
    ...
    <div max-width>
      <h3 v-if="mode === 'create'">Создать рецепт</h3>
      <h3 v-if="mode === 'update'">Редактировать рецепт</h3>

      <tc-segments :dark="$store.getters.darkmode" v-model="segment">
        <tc-segment-item title="Информация" />
        <tc-segment-item title="Сведения" />
        <tc-segment-item title="Дополнительно" />
      </tc-segments>
      ...
      <FHAppear>
        <div v-if="segment == 1">
          ...
          <div class="fh-graph-card">
            <div class="title" small>Новый ингредиент</div>
            <tc-input
          ...
          <div class="fh-graph-card">
            <div class="title" small>Новая характеристика</div>
            <tc-input
              placeholder="Название"
          ...
          <div class="title">Этапы готовки</div>
          <tc-table :dark="$store.getters.darkmode">
